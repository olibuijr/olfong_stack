...
class UnicontaService {
  constructor(config = {}) {
    ...
    this.client.interceptors.response.use(
      (response) => response,
      (error) => {
        console.error('Uniconta API Error:', error.response?.data || error.message);
        return Promise.reject(error);
      }
    );
  }
  ...
  /**
   * Test the connection to Uniconta API
   */
  ...
  /**
   * Authenticate with Uniconta API
   */
  async authenticate() {
    try {
  ...
  async syncOrder(orderData) {
    try {
      ...
      return {
        success: true,
        data: response.data,
        unicontaOrderId: response.data.id
      };
    } catch (error) {
      return {
        success: false,
  ...
  /**
   * Get order status from Uniconta
   */
  async getOrderStatus(unicontaOrderId) {
    try {
      const response = await this.client.get(`/api/orders/${unicontaOrderId}`);
  ...
  /**
   * Update order status in Uniconta
   */
  async updateOrderStatus(unicontaOrderId, status, notes = '') {
    try {
      const response = await this.client.patch(`/api/orders/${unicontaOrderId}`, {
        status,
        notes
      });
