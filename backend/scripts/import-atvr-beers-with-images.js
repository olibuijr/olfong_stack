...
// Configuration
const OLFONG_BEER_URL = 'https://olfong.is/voruflokkar/bjor/';
const UPLOADS_DIR = path.join(__dirname, '../uploads/products');
const ATVR_BASE_URL = 'http://localhost:5000/api/atvr';
const BEER_DATA_FILE = path.join(__dirname, '../../beer-import-data.json');
...
async function main() {
  console.log('🚀 Starting ATVR Beer Import with Olfong Images...');

  try {
    // Phase 1: Get ATVR beer products
    console.log('📊 Phase 1: Collecting ATVR beer products...');
    const atvrProducts = await getATVRBeerProducts();
    console.log(`Found ${atvrProducts.length} beer products from ATVR`);

    // Save ATVR data to file
    fs.writeFileSync(BEER_DATA_FILE, JSON.stringify(atvrProducts, null, 2));
    console.log(`💾 Saved ATVR data to ${BEER_DATA_FILE}`);

    // Phase 2: Get Olfong images
    console.log('🖼️ Phase 2: Collecting product images from Olfong...');
    const olfongImages = await getOlfongBeerImages();
    ...
    const results = await importProductsToDatabase(mergedProducts);
...
// Get beer products from ATVR API
async function getATVRBeerProducts() {
  try {
    console.log('Searching ATVR for beer products...');
    ...
    console.log(`Found ${products.length} initial beer products`);
...
// Scrape beer product images from Olfong
async function getOlfongBeerImages() {
  try {
    console.log('Fetching Olfong beer page with axios...');

    const response = await axios.get(OLFONG_BEER_URL, {
      headers: {
...
function generateImageFilename(productName) {
  if (!productName) return `beer-${Date.now()}.webp`;
...
async function importProductsToDatabase(products) {
  ...
  if (!beerCategory) {
    throw new Error('BEER category not found. Please run category seeding first.');
  }

  console.log(`Found beer category: ${beerCategory.name} (ID: ${beerCategory.id})`);
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      ...
      const productData = {
        ...
        description: product.description || `${product.name} - Quality beer from ${product.country || 'Iceland'}`,
...
module.exports = { main, getATVRBeerProducts, getOlfongBeerImages, matchProductsWithImages };